...
class Kudu {
    static oneDeploy(webPackage, queryParameters, scmUri, accessToken) {
        return __awaiter(this, void 0, void 0, function* () {
            let client = new KuduServiceClient_1.KuduServiceClient(scmUri, accessToken);
            let httpRequest = {
                method: 'POST',
                uri: client.getRequestUri(`/api/publish`, queryParameters),
                body: fs.createReadStream(webPackage)
            };
            let response = yield client.beginRequest(httpRequest, null, 'application/octet-stream');
            ...
            while (true) {
                ...
                let response = yield client.beginRequest(pollRequest);
                if (response.statusCode == 202)
                    continue;
